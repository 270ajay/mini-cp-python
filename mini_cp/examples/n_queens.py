...
def n_queens() -> "None":
    def branching_callback() -> "list[Callable[[], None]]":
        ...
        if idx == -1:
            ...
            v = qi.min()
            return [
                lambda: cp.post(factory.equal(qi, v)),  # left branch
                lambda: cp.post(factory.not_equal(qi, v=v)),  # right branch
            ]
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            # cp.post(factory.not_equal(factory.plus(q[i], j - 1), q[j]))
            # cp.post(factory.not_equal(factory.minus(q[i], j - 1), q[j]))

    search = factory.make_dfs(cp, branching_callback)
    search.on_solution(lambda: print(",".join([str(var) for var in q])))
    stats = search.solve(lambda statistics: statistics.number_of_solutions() == 1000)
